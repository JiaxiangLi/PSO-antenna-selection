function [R,S]=PSO_new_capacity(yita,RRH_matrix,USER_matrix,service_number,power_cell,distance_matrix,range)
NP = 100;                         % 种群个数
G = 200;                          % 迭代次数
...
for k=1:NP
    pbest(k, 1) = sum(fitness_for_multiple_user(yita,individual_best(:,:,k),service_number,power_cell));
end
...
for gen = 1:G
    ...
    % 个体历史最优
    for k=1:NP
        old_fitness = sum(fitness_for_multiple_user(yita,individual_best(:,:,k),service_number,power_cell));
        new_fitness = sum(fitness_for_multiple_user(yita,x(:,:,k),service_number,power_cell));
...
S=global_best;
R=real(global_best_fit);
% % if plot_y(1,1)==plot_y(1:gen)
% %     disp('迭代过程未更新全局最优')
% % end
% plot_x=1:G;
% plot(plot_x(1:gen),plot_y(1:gen),'r--','LineWidth',2);
% xlim([1 gen]);
% set(gca,'XTick',1:3:gen);
% % title(['Global Best vs Iteration Process']);
% % xlabel('Number of iterations') ;
% % ylabel('System downlink channel capacity (bit/s/Hz)') ;
% legend({'IDPSO-D','IDPSO-C'},'Location','southeast');
end
